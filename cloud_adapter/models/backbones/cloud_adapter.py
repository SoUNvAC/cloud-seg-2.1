...
def residual_stats(x_bnc, delta, scaled=None):
    """Per-layer residual statistics for the interaction module.

    Every quantity is a mean over the batch of a per-sample L2 norm taken over
    the flattened ``(N, C)`` token axis, so the reported residual ratio is

        ||alpha_l * delta_l||_2 / (||x_l||_2 + 1e-6)

    exactly as specified in the experiment protocol. ``torch.linalg.vector_norm``
    reduces without materialising a flattened copy, so this is cheap enough to
    run on every training iteration.

    Returns a detached 1-D tensor ``[x_norm, delta_norm, scaled_norm, ratio]``
    kept on the compute device; the caller materialises it only when logging.
    """
    x_norm = torch.linalg.vector_norm(x_bnc.detach(), dim=(1, 2)).mean()
    delta_norm = torch.linalg.vector_norm(delta.detach(), dim=(1, 2)).mean()
    if scaled is None:
        scaled_norm = delta_norm
    else:
        scaled_norm = torch.linalg.vector_norm(scaled.detach(), dim=(1, 2)).mean()
    ratio = scaled_norm / (x_norm + 1e-6)
    return torch.stack([x_norm, delta_norm, scaled_norm, ratio])
...
class ConvnextInteractiveModule(nn.Module):
    def __init__(self, emd_dim=1024, context_dim=256, rank_dim=None):
        ...
        # Diagnostics only; toggled by LayerScaleStatsHook. Never part of the
        # state_dict and never affects the forward result.
        self.collect_stats = False
        self.last_stats = None

    def _prepare(self, x, cache, index):
        """Cache selection, interpolation and the batch-first conversion."""
    ...
    def forward(self, x, cache, index):
        # x: 1024 2 1024
        x, cache = self._prepare(x, cache, index)
        delta = self.attn(x, cache)
        out = x + delta
        if self.collect_stats:
            self.last_stats = residual_stats(x, delta, None)
        return out.permute(1, 0, 2)


class ScaledConvnextInteractiveModule(ConvnextInteractiveModule):
    """Interaction module with a learnable residual scaling coefficient.

    ``alpha`` starts at ``init_scale`` and multiplies the cross-attention
    residual, so the module computes ``x + alpha * attn(x, cache)``. Each of the
    24 interaction layers owns an independent ``alpha`` (never a shared scalar).

    ``scale_type="scalar"`` gives one parameter per layer; ``"channel"`` gives
    one per embedding channel. In both cases the parameter lives under
    ``cloud_adapter`` in the module tree, so it is picked up by the
    ``PEFTOptimWrapperConstructor`` (requires_grad is set by ``set_requires_grad``)
    and kept by ``CloudAdapterDinoVisionTransformer.state_dict``, which drops
    every key that does not contain ``cloud_adapter``.
    """

    def __init__(self, emd_dim=1024, context_dim=256, rank_dim=None,
                 init_scale=0.1, scale_type="scalar"):
        super().__init__(emd_dim, context_dim, rank_dim)
        ...
        if scale_type == "scalar":
            ...
            raise ValueError(
                f"scale_type must be 'scalar' or 'channel', but got {scale_type}"
            )
    ...
    def forward(self, x, cache, index):
        ...
        if self.alpha.ndim == 0:
            scaled = self.alpha * delta
        else:
            scaled = self.alpha.view(1, 1, -1) * delta
        out = x + scaled
        if self.collect_stats:
            self.last_stats = residual_stats(x, delta, scaled)
        return out.permute(1, 0, 2)
...
class CloudAdapter(nn.Module):
    def __init__(self,
        ...
        if int_type == "convnext":
            if use_layer_scale:
                # One independent alpha per interaction layer. The generator
                # expression builds a distinct module per layer, so the alphas
                # are never tied/shared.
