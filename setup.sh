...
conda run --no-capture-output --name "$ENV_NAME" python -c \
    "import importlib.metadata as metadata; import pkg_resources, setuptools, torch, mmcv, mmengine, mmseg, mmdet, xformers, future, tensorboard; from torch.utils.tensorboard import SummaryWriter; from mmcv.ops import MultiScaleDeformableAttention; import cloud_adapter; expected={'setuptools':'81.0.0','torch':'2.1.2','mmcv':'2.1.0','mmengine':'0.10.4','mmseg':'1.2.2','mmdet':'3.3.0','xformers':'0.0.23.post1','future':'1.0.0','tensorboard':'2.21.0'}; actual={'setuptools':setuptools.__version__,'torch':torch.__version__.split('+')[0],'mmcv':mmcv.__version__,'mmengine':mmengine.__version__,'mmseg':mmseg.__version__,'mmdet':mmdet.__version__,'xformers':xformers.__version__,'future':metadata.version('future'),'tensorboard':metadata.version('tensorboard')}; mismatched={k:(actual[k],v) for k,v in expected.items() if actual[k] != v}; assert not mismatched, f'version mismatch: {mismatched}'; print('versions:', actual); print('CUDA available:', torch.cuda.is_available()); print('TensorBoard backend import: OK'); print('Cloud-Adapter import: OK')"
